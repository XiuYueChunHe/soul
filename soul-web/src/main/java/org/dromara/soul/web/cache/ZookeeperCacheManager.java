...
public class ZookeeperCacheManager implements CommandLineRunner, DisposableBean {
    ...
    private void loadPluginWatcher() {
        StaticLog.debug("从zk加载Plugin节点数据开始");
        Arrays.stream(PluginEnum.values()).forEach(pluginEnum -> {

            StaticLog.debug(String.format("从zk加载Plugin[%s]节点数据", JSON.toJSON(pluginEnum)));
            ...
            StaticLog.debug("读取zk节点path", U.format("pluginPath", pluginPath));

            //将插件数据缓存到PLUGIN_MAP
            PluginZkDTO pluginZkDTO = zkClient.readData(pluginPath);
            Optional.ofNullable(pluginZkDTO).ifPresent(d -> {
                StaticLog.debug("读取zk节点plugin数据存在,保存到PLUGIN_MAP", U.format("pluginZkDTO", JSON.toJSON(pluginZkDTO)));
                PLUGIN_MAP.put(pluginEnum.getName(), pluginZkDTO);
            });

            StaticLog.debug("监听zk节点plugin数据更新操作");
            ...
            zkClient.subscribeDataChanges(pluginPath, new IZkDataListener() {
                ...
                public void handleDataChange(final String dataPath, final Object data) {
                    //如果数据更新,则更新plugin缓存MAP
                    StaticLog.debug("zk节点plugin数据更新", U.format("dataPath", dataPath, "pluginZkDTO", JSON.toJSON(data)));
                    Optional.ofNullable(data)
                            .ifPresent(o -> {
                                PluginZkDTO dto = (PluginZkDTO) o;
                                PLUGIN_MAP.put(dto.getName(), dto);
                                StaticLog.debug("zk节点plugin更新数据保存到PLUGIN_MAP", U.format("pluginZkDTO", JSON.toJSON(dto)));
                            });
                }
                ...
                public void handleDataDeleted(final String dataPath) {
                    ...
                    StaticLog.debug("zk节点plugin数据删除,同步删除PLUGIN_MAP中缓存数据", U.format("dataPath", dataPath, "pluginZkDTO", JSON.toJSON(remove)));
                }
            });
    ...
    private void loadSelectorWatcher() {
        ...
        StaticLog.debug("从zk加载Selector节点数据开始");

        Arrays.stream(PluginEnum.values()).forEach(pluginEnum -> {

            StaticLog.debug(String.format("从zk加载Selector节点数据,所属plugin[%s]", JSON.toJSON(pluginEnum)));

            //获取选择器的节点
            String pluginEnumName = pluginEnum.getName();
            String selectorParentPath = ZkPathConstants.buildSelectorParentPath(pluginEnumName);
            StaticLog.debug("读取zk节点path", U.format("selectorParentPath", selectorParentPath));
            ...
            if (CollectionUtils.isNotEmpty(selectorPathList)) {
                StaticLog.debug("初始化监听所有Selector节点数据,并注册每个Selector节点数据监听器", U.format("pluginEnumName", pluginEnumName, "selectorPathList", JSON.toJSON(selectorPathList)));
                selectorPathList.forEach(selectorPath -> {
                    String selectorRealPath = buildRealPath(selectorParentPath, selectorPath);
                    StaticLog.debug("从zk获取Selector节点数据", U.format("selectorPath", JSON.toJSON(selectorPath), "selectorRealPath", selectorRealPath));
                    ...
                    Optional.ofNullable(selectorZkDTO)
                            .ifPresent(dto -> {
                                ...
                                StaticLog.debug("Selector节点数据存在", U.format("pluginEnumName", pluginEnumName, "selectorZkDTO", JSON.toJSON(selectorZkDTO)));
            ...
            StaticLog.debug("注册当前plugin所属的所有Selector节点数据更新事件", U.format("pluginEnumName", pluginEnumName, "selectorParentPath", selectorParentPath));

            zkClient.subscribeChildChanges(selectorParentPath, (parentPath, selectorsPath) -> {
                if (CollectionUtils.isNotEmpty(selectorsPath)) {
                    StaticLog.debug("监听到当前plugin所属的Selector节点数据发生更新", U.format("pluginEnumName", pluginEnumName, "parentPath", parentPath, "selectorsPath", JSON.toJSON(selectorsPath)));
    ...
    private void loadRuleWatcher() {
        ...
        StaticLog.debug("从zk加载Rule节点数据开始");

        Arrays.stream(PluginEnum.values()).forEach(pluginEnum -> {

            StaticLog.debug(String.format("从zk加载Rule节点数据,所属plugin[%s]", JSON.toJSON(pluginEnum)));
            String pluginEnumName = pluginEnum.getName();
            ...
            StaticLog.debug("读取zk节点path", U.format("ruleParent", ruleParent));

            final List<String> rulePathList = zkClient.getChildren(ruleParent);


            if (CollectionUtils.isNotEmpty(rulePathList)) {
                StaticLog.debug("初始化监听所有Rule节点数据,并注册每个Rule节点数据监听器", U.format("pluginEnumName", pluginEnumName, "rulePathList", JSON.toJSON(rulePathList)));
                rulePathList.forEach(rulePath -> {
                    String ruleRealPath = buildRealPath(ruleParent, rulePath);
                    StaticLog.debug("从zk获取Rule节点数据", U.format("rulePath", JSON.toJSON(rulePath), "ruleRealPath", ruleRealPath));
                    final RuleZkDTO ruleZkDTO = zkClient.readData(ruleRealPath);
                    Optional.ofNullable(ruleZkDTO)
                            .ifPresent(dto -> {
                                String selectorId = dto.getSelectorId();
                                StaticLog.debug("Rule节点数据存在", U.format("pluginEnumName", pluginEnumName, "ruleZkDTO", JSON.toJSON(ruleZkDTO)));
        ...
        Optional.ofNullable(pluginName)
                .ifPresent(pluginName2 -> {
                    ...
                    if (selectorZkDTO.getPluginName().equals(PluginEnum.DIVIDE.getName())) {
                        StaticLog.debug("向UpstreamCacheManager注册SelectorZkDTO", U.format("pluginName", pluginName, "selectorZkDTO", JSON.toJSON(selectorZkDTO)));
                        UpstreamCacheManager.submit(selectorZkDTO);
                    }

                    //向SELECTOR_MAP添加SelectorZkDTO数据
                    StaticLog.debug("向SELECTOR_MAP添加SelectorZkDTO数据");
                    if (SELECTOR_MAP.containsKey(pluginName2)) {
                        final List<SelectorZkDTO> selectorZkDTOList = SELECTOR_MAP.get(pluginName);
                        StaticLog.debug("SELECTOR_MAP更新前selectorZkDTOList", U.format("selectorZkDTOList", JSON.toJSON(selectorZkDTOList)));
                        ...
                        final List<SelectorZkDTO> collect = resultList.stream()
                                ...
                                .collect(Collectors.toList());
                        StaticLog.debug("SELECTOR_MAP更新后selectorZkDTOList", U.format("selectorZkDTOList", JSON.toJSON(collect)));
                        SELECTOR_MAP.put(pluginName, collect);
                    } else {
                        //直接向SELECTOR_MAP添加SelectorZkDTO数据
                        StaticLog.debug("SELECTOR_MAP中不存在当前plugin对应的selectorZkDTOList,直接添加");
    ...
    private void subscribeSelectorDataChanges(final String path) {
        StaticLog.debug("注册Selector节点数据更新监听事件", U.format("Selector zk path", path));
        zkClient.subscribeDataChanges(path, new IZkDataListener() {
            ...
            public void handleDataChange(final String dataPath, final Object data) {
                ...
                Optional.ofNullable(data)
                        .ifPresent(d -> {
                            ...
                            StaticLog.debug("Selector节点数据更新事件", U.format("dataPath", dataPath, "SelectorZkDTO", JSON.toJSON(selectorZkDTO)));
            ...
            public void handleDataDeleted(final String dataPath) {
                ...
                Optional.of(key).ifPresent(k -> {
                    ...
                    StaticLog.debug("Selector节点数据删除事件", U.format("dataPath", dataPath));
                    //从当前plugin对应的selectorZkDTOList中移除该selectorZkDTO
                    StaticLog.debug("SELECTOR_MAP删除前selectorZkDTOList", U.format("selectorZkDTOList", JSON.toJSON(selectorZkDTOList)));
                    selectorZkDTOList.removeIf(e -> e.getId().equals(id));
                    StaticLog.debug("SELECTOR_MAP删除后selectorZkDTOList", U.format("selectorZkDTOList", JSON.toJSON(selectorZkDTOList)));
                });
            }
        });
    }
    ...
    private void setRuleMapByKey(final String selectorId, final RuleZkDTO ruleZkDTO) {
        Optional.ofNullable(selectorId)
                .ifPresent(k -> {
                    StaticLog.debug("向RULE_MAP添加RuleZkDTO数据", U.format("selectorId", selectorId, "ruleZkDTO", JSON.toJSON(ruleZkDTO)));
                    if (RULE_MAP.containsKey(k)) {
                        final List<RuleZkDTO> ruleZkDTOList = RULE_MAP.get(selectorId);
                        StaticLog.debug("RULE_MAP更新前ruleZkDTOList", U.format("ruleZkDTOList", JSON.toJSON(ruleZkDTOList)));
                        ...
                        StaticLog.debug("RULE_MAP更新后ruleZkDTOList", U.format("ruleZkDTOList", JSON.toJSON(collect)));
                    } else {
                        StaticLog.debug("RULE_MAP中不存在当前selectorId对应的ruleZkDTOList,直接添加");
    ...
    private void subscribeRuleDataChanges(final String ruleRealPath) {
        zkClient.subscribeDataChanges(ruleRealPath, new IZkDataListener() {
            ...
            public void handleDataChange(final String dataPath, final Object data) {
                Optional.ofNullable(data)
                        .ifPresent(d -> {
                            ...
                            StaticLog.debug("Rule节点数据更新事件", U.format("dataPath", dataPath, "ruleZkDTO", JSON.toJSON(ruleZkDTO)));
            ...
            @Override
            public void handleDataDeleted(final String dataPath) {
                StaticLog.debug("Rule节点数据删除事件", U.format("dataPath", dataPath));
                ...
                StaticLog.debug("根据ruleId移除UpstreamCacheManager中缓存的DivideUpstream", U.format("selectorId", selectorId, "ruleId", ruleId));
                ...
                Optional.ofNullable(selectorId).ifPresent(selectorIdd -> {
                    ...
                    StaticLog.debug("RULE_MAP删除前ruleZkDTOList", U.format("ruleZkDTOList", JSON.toJSON(ruleZkDTOList)));
                    ruleZkDTOList.removeIf(e -> e.getId().equals(ruleId));
                    StaticLog.debug("RULE_MAP删除后ruleZkDTOList", U.format("ruleZkDTOList", JSON.toJSON(ruleZkDTOList)));
                });
            }
        });
    }
